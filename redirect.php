...
$authtype = 'mykooboauth2';
$debug = get_config('auth/' . $authtype, 'debug');

$goto = get_login_url();
if (!is_enabled_auth($authtype)) {
    if ($debug) {
        error_log('[Mykoob OAuth2] Trying login with disabled auth plugin.');
    }
...
if (empty($code)) {
    ...
    if (!empty($error)) {
        if ($debug) {
            error_log('[Mykoob OAuth2] Authentication failed. Error: "' . $error . '"/"' . $errordesc . '"');
        }
    }
